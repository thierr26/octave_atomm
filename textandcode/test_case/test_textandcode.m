...
## -*- texinfo -*-
## @deftypefn {Function File} {@var{s} =} test_textandcode ()
##
## Run test case for toolbox "textandcode", return results as a structure.
##
## @code{@var{s} = test_textandcode ()} actually runs a
...
## "test_textandcode" (function name, given by function @code{mfilename}).
##
## @item @var{test_routine}
## Cell array of handles to local functions (test routines) written
## specifically to test the toolbox "textandcode".
## @end table
##
...
function s = test_textandcode
    ...
    testRoutine = {...
        ...
        @strip_comment_from_line_wrong_arg_3, ...
        ...
        @strip_comment_from_line_nocheck_false, ...
...
function strip_comment_from_line_wrong_arg_3

    strip_comment_from_line('# abc', '%#', 1);
...
function ret = strip_comment_from_line_nocheck_false

    ret = isequal(strip_comment_from_line('% abc', '#%', false), '');
